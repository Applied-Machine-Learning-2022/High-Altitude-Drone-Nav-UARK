...
@tf.function
def _random_center(mask_dim_length, image_dim_length, batch_size, generator):
    if mask_dim_length >= image_dim_length:
        return tf.tile([image_dim_length // 2], [batch_size])
    half_mask_dim_length = mask_dim_length // 2
    return generator.uniform(shape=[batch_size], minval=-half_mask_dim_length, maxval=image_dim_length + half_mask_dim_length, dtype=tf.int32)
...
def random_cutout(
    ...
    generator: tf.random.Generator = tf.random.Generator.from_non_deterministic_state(),
) -> tf.Tensor:
    ...
    cutout_center_height = _random_center(mask_size[0], image_height, batch_size, generator)
    cutout_center_width = _random_center(mask_size[1], image_width, batch_size, generator)
