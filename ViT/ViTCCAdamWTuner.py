# import pip install --upgrade tensorflow
...
from vit2_model import VisionTransformer
from coord import CoordinateChannel2D
def tuner_builder(hp):


    patch_size = hp.Choice('patch_size', values=[14,28,56])
    num_layers = hp.Int('num_layers', 4,24)
    d_model = hp.Int('d_model',512,4096, step=512)
    num_heads = hp.Choice('num_heads', values=[8,16,32,64])
    mlp_dim = hp.Choice('mlp_dim',values = [1024,2048,4096])
    ...
    coorded = CoordinateChannel2D()(input_layer)
    ...
    new_model = tf.keras.Model(inputs=input_layer, outputs=finaldist)
    ...
    cycle_length = hp.Int('cycle_length', 60000, 260000, step=40000)

    cyclical_learning_rate = CyclicalLearningRate(
        initial_learning_rate=initial_lr,
        maximal_learning_rate=lr_factor * initial_lr,
        step_size=cycle_length,
        scale_fn=lambda x: (1 / (1.01 ** (x-1))),
        scale_mode='cycle')

    adamopt = tfa.optimizers.AdamW(learning_rate=cyclical_learning_rate, weight_decay=weight_decay, clipvalue=1)
...
tuner = kt.Hyperband(tuner_builder,
                     objective=kt.Objective("val_loss", direction="min"),
                     max_epochs=1000,
                     factor=6,
                     directory='ViTCCTuner',
                     project_name='ViTCCTuner')
...
checkpoint_filepath = str(workingpath / f'{testname:s}' / (f'{testname:s}' + '_{epoch:02d}_{val_rmse:.4f}_{val_loss:.4f}.hdf5'))
...
callbacks_list = [checkpoint, csv_logger, terminateOnNaNCall, stop_early]

# callbacks_list = [checkpoint]

tuner.search(train_ds, shuffle=True, validation_data=valid_ds,
                    max_queue_size=16,
...
# stddevs = []
#
# for element in valid_ds.as_numpy_iterator():
#     trues.append(element[1])
#     dist = new_model(element[0])
#     means.append(dist.mean().numpy())
#     stddevs.append(dist.stddev().numpy())
#
# with open(str(workingpath.joinpath(f'{testname:s}', f'valid_trues{testname}.pkl')), 'wb') as file_pi:
#     pickle.dump(trues, file_pi)
#
# with open(str(workingpath.joinpath(f'{testname:s}', f'valid_means{testname}.pkl')), 'wb') as file_pi:
#     pickle.dump(means, file_pi)
#
# with open(str(workingpath.joinpath(f'{testname:s}', f'valid_stddevs{testname}.pkl')), 'wb') as file_pi:
